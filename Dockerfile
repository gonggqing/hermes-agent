...
#
# Download all tarballs inside the build layer rather than with remote `ADD`.
# BuildKit's remote-ADD fetch has no reliable retry path and can abort the
# entire build on a transient GitHub release-asset EOF. curl lets local builds
# retry transport errors while the checksums below preserve supply-chain
# integrity. A single arch-keyed URL also avoids downloading dead bytes.
#
...
RUN set -eu; \
    ...
    curl_args="-fsSL --retry 10 --retry-all-errors --retry-delay 2 --connect-timeout 20"; \
    curl ${curl_args} -o /tmp/s6-overlay-noarch.tar.xz \
        "${S6_OVERLAY_BASE_URL}/v${S6_OVERLAY_VERSION}/s6-overlay-noarch.tar.xz"; \
    curl ${curl_args} -o /tmp/s6-overlay-symlinks-noarch.tar.xz \
        "${S6_OVERLAY_BASE_URL}/v${S6_OVERLAY_VERSION}/s6-overlay-symlinks-noarch.tar.xz"; \
    curl ${curl_args} -o /tmp/s6-overlay-arch.tar.xz \
        "${S6_OVERLAY_BASE_URL}/v${S6_OVERLAY_VERSION}/s6-overlay-${s6_arch}.tar.xz"; \
    { \

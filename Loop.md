...
- [x] **US/HK/CN instrument search:** type-ahead by partial symbol or company/fund name; results identify symbol, display name, exchange, currency and security type (stock/ETF/fund), behind a mockable search provider with cached metadata and no network in tests. — DONE: `instruments.py` (`normalize_symbol`, static + held-instrument sources, live `YFinanceInstrumentProvider` for US/HK/CN exchange instruments, live `EastmoneyInstrumentProvider` for Chinese names/funds, `CachedInstrumentSearch`); `/v1/instruments/search`. Finance Bot trade messages MUST pass through the configured high-speed LLM as a stateless structured extractor, then through this provider and deterministic holdings/account checks; unknown or ambiguous instruments create no draft, and only a human-confirmed card can write the journal.
...
- 2026-07-14 — **Post-push CI fix + as-of-bar §5.10 fix (5 commits).** (a) **CI green again** (fork main): the upstream sync's blocking `ruff check .` failure was 21 pre-existing `trader/` findings — root ruff recurses into trader/ under trader's own config (F401/E731/E741/F841) — all cleared; the `test_prompt_builder` soul test asserted upstream "Hermes Agent" but our fork seeds "Hermes Finance" (default_soul.py) → assertion updated to our persona. (b) **`package-lock.json` corruption from the merge**: git's textual auto-merge silently DROPPED 33 build-dep entries (all 26 `@esbuild/*` binaries + electron build deps) — CI's `npm ci --ignore-scripts` then couldn't install `@esbuild/linux-x64`, failing the apps/desktop + ui-tui BUILDS. Spliced the exact 33 entries back from upstream/main (macOS `npm install` prunes cross-platform optionals, so can't regenerate); `npm ci --dry-run` validates. Lesson recorded in memory. (c) **Verdict as-of-bar (Loop.md §5.10 completeness gap the human caught)**: a verdict's close/SMA/RSI reflect the LAST BAR analyzed, but only the runtime `ts` was recorded, not the data bar date — over a weekend a 6/30-anchored "0.80 long" (close 155.38) looked wrong vs a 7/14 live price (152.46). `Signal.as_of_bar` now carries `bars[-1].ts` (TechnicalAgent sets it; DebateAgent inherits the freshest; persisted via features_json — no schema migration); surfaced in the brief SignalView, `/v1/analyze` (`as_of_bar` + `render_analysis_zh` header "数据截至 <date>", 收 not 现价), and the Web + Desktop research signal cards. 1107 trader + 97 web + 1688 desktop tests green; CI all-required-checks pass.
- 2026-07-14 — **#41 research: KR semiconductor market + manual research trigger + upstream sync (7 commits).** (a) **KR semiconductor research session** (`kr_watchlist.py`: Samsung 005930.KS / SK Hynix 000660.KS memory giants + HBM chain; `KR_SCHEDULE` Asia/Seoul 09:30→14:30→15:00 near KR close; `/v1/research/brief?market=kr`) — KR semi sentiment leads/transfers to CN; UK/JP dropped (human directive). Activated as a Web + Desktop research desk (moved from disabled placeholder → active; UK/JP placeholders removed). Live-verified (Samsung/SK Hynix/KOSPI). (b) **Manual "Run research now" button** (both surfaces) → `POST /v1/research/run?market=cn|kr` → `ResearchSession.run_now()` re-runs monitors + rebuilds the brief with FRESH data (not just a refetch); read-only so UNGATED (unlike the human-gated trading session). (c) **CN/HK per-region split** confirmed done (frontend `partitionCnBrief` by suffix) + hardened: `Mover.region` now carries CN/HK/KR/US from the symbol suffix (backend-structured). (d) **Reviewed upstream Hermes sync merge** — upstream/main 861d69c7b (196 commits, 437 files, ZERO conflicts), Finance boundaries verified intact, merged to main + pushed; report `docs/upstream-sync/2026-07-14.md`. Validation: trader 1101 · web typecheck+97 · desktop typecheck+1688 · RiskEngine 100% branch.
- 2026-07-14 — **Phase-1 broker backbone + P0.95 go-live gate (7 commits).** The Phase-1 *code* is built + validated ENTIRELY OFFLINE so IBKR slots in the moment the account funds: **`IBKRBroker`** (`ibkr_broker.py`) behind a neutral `IBClient` port (place/cancel/trades/fills/positions/account) — full place→partial→fill→cancel→reject/bracket-OCA lifecycle driven by a mock IB transport; `_IbAsyncClient` imports `ib_async` LAZILY in `connect()` so constructing the broker opens no socket + imports nothing (both asserted); CASH account spends only `SettledCash` (no free-riding), fails closed locally before IBKR; IBKR **paper** account tags `Mode.PAPER` / live tags `Mode.LIVE` (paper-sim testing never pollutes the live-money ledger). **`broker_factory.build_broker`** — the one place mapping `BROKER=<backend>`→adapter; the paper/live *account* flag is DERIVED from the triple gate (`live_orders_allowed`), never the port, so an un-gated config can only reach paper and a live IBKR port under it fails closed at construction; `__main__` Phase-0 hard stop (`paper only`) removed (live-order gate intact), clear ib_async install hint; `live_orders_allowed` threaded DailyLoop→ExecutionEngine (default False, fail-closed); optional `[ibkr]` extra. **Kill-switch** (`killswitch.py`) — filesystem HALT flag next to the DB: presence=engaged, survives restart, fails CLOSED, touch-able out-of-band; wired as the highest-precedence `assess_health` gate → `entries_allowed=False` which the pure/100%-branch RiskEngine already enforces; does NOT cancel protective stops (§4). HTTP `GET /v1/killswitch` + `POST /v1/killswitch/{engage,release}` (engage=any surface incl. agent; release=HUMAN-only 403 for system/LLM) + human-only `POST /v1/orders/cancel-all` (`ExecutionEngine.cancel_all_orders(include_protection=)`); CLI `kill`/`release`/`killswitch-status`. **Go-live runbook** `docs/finance/go-live-runbook.md` (prereqs, .env triple-gate reference, mandatory paper-first dry run, cutover, kill-switch drill, emergency + rollback, 5-layer defence-in-depth appendix). **Regime-segmented walk-forward** (`regime_analysis.py`) — buckets OOS folds by test-window regime (bull/bear/chop from benchmark return), re-aggregates per regime, `passes(min_regimes=2,require_all_nonneg=)` coverage gate; `WalkForwardFold` now carries `test_window`+`test_trades`. **Broker parity harness** — PaperBroker↔IBKRBroker substitutability (accept/reject/idempotency/cancel/cash contract; broker_ref documented as a broker-specific difference). +113 trader tests → **1050 green**; RiskEngine still 100% branch. Still ⛔ pre-live: reviewed weekend upstream sync merge, real IBKR-paper dry run (needs TWS), ≥20 paper-day exit, guardrail audit, **human sign-off** — NOT self-advanced.

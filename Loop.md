...
- **Current operating priority (September 2026):** stabilize the twice-daily research product before resuming approval volume: fresh non-repeated evidence, market-specific primary-model judgment, explainable stock selection, longitudinal thesis changes and auditable outcome quality. Do not expand automatic trading while these research gates remain open; existing execution safety fixes continue, but Quant remains deferred.
...
- [x] Bind each scheduled brief to a stable market/date/edition ID and the same-run evidence timestamp; quarantine stale prose, upsert duplicate presentation snapshots, and keep a visible primary-model failure state instead of pairing old narrative with new prices.
...
- [x] Upgrade the primary brief contract to v7: every priority instrument must explain why it matters now, its industry/supply-chain or portfolio role, at least two distinct evidence pillars, fresh catalysts, strongest counter-case, horizon and observable invalidation; Web/Desktop show material thesis changes first in one flat Hermes-native reading flow.
- [x] Stop confirmed historical price gaps from retrying forever: when later daily bars prove the due session is absent, persist an auditable unscorable suspension/non-trading/source-gap outcome instead of repeating ten-minute warnings.
- [ ] Complete longitudinal thesis tracking across editions (`new → strengthened → unchanged → weakened → invalidated`) with a per-symbol history view, material-change alerts and explicit links from each state transition to the evidence revision that caused it.
- [ ] Calibrate prediction producers and reduce noise: merge semantically equivalent same-instrument views for presentation, demote unsupported technical-only claims, retain raw producer records for audit, and publish confidence intervals before using accuracy to tune selection prompts.
...
- [x] Make prediction evaluation type-aware: listed instruments/indices use exact canonical symbols, supported indicators use explicit price series, themes use valid listed-leader baskets, and synthetic labels become permanently unscorable rather than infinite Yahoo retries.
...
- [x] Enable the local HK Paper loop in Compose and verify approved SEHK candidates flow through HKD reservation, DAY entry fill, ledger sync and GTC stop/take-profit protection; this is simulation only and does not satisfy connected IBKR Paper acceptance.
- [x] Default portfolio aggregate/valuation reads to real accounts, keep paper views explicit, and refresh marks for currently open live holdings at startup and before each scheduled brief without blending account environments.
...
- 2026-09-17 — **Research-first brief v7 and evaluator-noise reduction.** Kept automatic-trading expansion paused; made each primary-model priority view expose why-now, market/industry role, multi-pillar evidence, catalysts, counter-case and invalidation, reordered Web/Desktop around material thesis changes using the shared Hermes visual language and all Desktop core locales, and retired confirmed historical close gaps as auditable unscorable outcomes instead of endless retries.
- 2026-09-09 — **Research integrity and regional-paper recovery.** Synced official main, made scheduled briefs edition/evidence-bound with stale-prose quarantine and canonical deduplication, pinned final synthesis to the Hermes primary provider/model, retired malformed prediction entities in favor of exact index/indicator/theme-basket evaluation, isolated live portfolio valuation with automatic held-mark refresh, and enabled/verified the HKD local PaperBroker chain; connected IBKR Paper and multi-day runtime soak remain open.

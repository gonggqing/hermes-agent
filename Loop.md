...
- **Goal:** build and harden the REAL broker adapter + order lifecycle so IBKR slots in cleanly the moment the account funds — validated fully offline against mocks (and IBKR paper when TWS is available). This front-loads the Phase-1 *code* during the ~2-week account wait, leaving Phase 1 to be "connect + validate + go tiny-live."
- **Build:** `IBKRBroker` (ib_async) implementing `BrokerInterface` — bracket/OCA mapping, MOC/LOC/GTC, **client-order-id idempotency**, partial/reject/cancel/timeout handling, reconnection + pacing, and **T+1 settled-cash** tracking (HK cash account, §2); a mock IB transport so the whole place→partial→fill→cancel→reject lifecycle is unit-tested offline (same suite the PaperBroker passes); order-state **reconciliation on restart**; a **paper↔live ledger-comparison harness** (measures the sim→real gap) reusing the backtest code paths.
- **Exit:** `IBKRBroker` passes the mock-exchange lifecycle tests offline; reconciliation detects drift; the comparison harness runs on paper data; still NO live orders (the §3 triple gate `HUMAN_CONFIRM ∧ BROKER≠paper ∧ ¬DRY_RUN` is untouched).
...
- 2026-07-13 — **Professional TradingView-style K-chart (klinecharts)** on Desktop + Web (human directive: "more professional like tradingview", "I need to learn from the k-chart"). Adopted **klinecharts 10.0.0** (new dep; reverses the earlier no-chart-lib call — the requirements outgrew inline SVG) — it owns candles, TradingView-style axes/ticks, crosshair, value tooltip, drag-pan + zoom. Rebuilt the watch module into ONE large chart (~480px) per page: **symbol dropdown top-left**, **timeframe switcher bottom-right (1D/5D/D/M/Y)** each with its own auto date range (5m/30m/1d/1wk/1mo), x-axis granularity follows the data. **Indicator toolkit** (popover) each with a SHORT educational description (learning tool): MA (MA20/30 CURVES, default) / EMA / BOLL overlays; VOL (default) / MACD / RSI / KDJ sub-panes. Currency on the price axis ($ / ¥ + unit) + AU9999 stays derived. Theme-reactive; dispose-on-unmount; graceful no-data. Removed the old inline-SVG candlestick/MA/crosshair. Read-only preserved; i18n desktop en+zh+zh-hant+ja + web en+zh. Desktop typecheck + web typecheck/build green (klinecharts bundles cleanly); runtime rendering to be visually confirmed by the human. (NOTE for future UI: the finance chart is now klinecharts, not inline SVG.)
- 2026-07-13 — **Finance chart pro-iteration + currency/AU9999** (human directives; parallel workflows, verified). Chart: **timeframe switcher 1D/5D/Day/Week/Month** (→ 5m/30m/1d/1wk/1mo — DataFeed extended to serve intraday+monthly intervals, commit a243e4d14, +6 tests, suite 747), cached per (symbol,timeframe) with keepPreviousData; **MA20/MA30 均线** overlay lines (amber/blue) + legend; **removed the LIVE badge + live-price poll**; **desktop-only sidebar icons** (flag glyphs for US/China/HK + muted UK/KR/JP; asset marks Gold/Oil/Rates/Crypto, messaging-avatar style; web sidebar untouched). Prices now show **currency + unit**: `4079 $ / 盎司`, `8.42 ¥ / 股`, `78.5 $ / 桶`, `4.30 %`, `$67,000` (localized units). **AU9999 (国内金价)** added to Gold as a DERIVED symbol (SGE spot not on Yahoo): `GC=F(USD/oz) × CNY=X(CNY/USD) / 31.1035` → ~890 ¥/克 quote + a GC=F-derived candlestick rescaled to ¥/gram, labeled derived, graceful no-data. Real SGE AU9999 + other domestic feeds queued for Phase 0.9 (§7 P0.9 / task #41). Read-only preserved; i18n desktop en+zh+zh-hant+ja + web en+zh; typecheck/build green.
- 2026-07-13 — **Finance UI iteration (pro charts + bottom bar)** on Desktop + Web (human directive; parallel workflow, verified). Candlestick hover (TradingView-style crosshair + Date/O/H/L/C/vol tooltip from cached bars); bars CACHED per symbol (no more load-then-refresh flash) with the LAST candle growing live off a lightweight ~4s quote poll + CSS transition + "LIVE" tag (heavy bars endpoint NOT re-polled — yfinance is ~15-min delayed); first-load reuses the shared capabilities-loading skeleton; ONE Analyze button per page (top-right, no icon, symbols became selectable cards); service status/breaker/loop/as-of/refresh moved to a BOTTOM utility bar; paper/live is now a SINGLE toggle button showing current mode (not two tabs). Dependency-free (inline SVG + DOM overlays; no chart lib). Read-only preserved (approve/edit/reject still queue-only). i18n desktop en+zh+zh-hant+ja + web en+zh; typecheck/build green; web deployed. Also: the unpinned `daily-hermes-trade-review` cron correctly SKIPPED (spend guard) after the MiniMax model switch — resume by pinning it to minimax-cn/MiniMax-M3 via the bot's cronjob tool.
- 2026-07-13 — **Finance tab redesigned to a messaging-style master-detail** (human directive) on Desktop + Web. Each of Research / Action Queue / Portfolio is now a left sidebar (grouped list) → right detail → **bottom paper/live switcher**. Research sidebar = **Markets** (US/China/HK active + UK/Korea/Japan disabled "Phase 0.9" placeholders) + **Watch modules** (Gold incl. Shanghai gold ETF, Oil WTI/Brent, Rates & Bonds, Crypto watch-only): a market → its research brief, a module → quote + inline-SVG candlestick + lazy Analyze (READ-ONLY). Queue = pending-candidate list + the unchanged approve/edit/reject detail (switcher filters mode, an action's mode is fixed). Portfolio = positions/account rows → detail. NEW web/desktop clients `financeQuote`/`financeBars`/`financeAnalyze` over the existing `/v1/quote|bars|analyze`. **China vs HK are frontend-partitioned from the ONE combined CN brief** (movers/signals by `.SS/.SZ` vs `.HK`; regime/news/themes shared) with a Phase-0.9 note — proper per-region briefs + UK/KR/JP sessions + real domestic-gold data are queued as backend follow-ups. §3 held: no order/approve path added anywhere new (verified). Desktop reused MasterDetail/ListColumn/DetailColumn/SegmentedControl; web hand-built a grid master-detail. i18n desktop en+zh+zh-hant+ja (compile-enforced) + web en+zh. Desktop typecheck + web typecheck/build green; parallel workflow. (Desktop app = separate Electron build the human runs; web ships in the docker image.)
- 2026-07-13 — **Model switch (MiniMax) + deeper research ingestion.** Model config (human directive): daily/main model → **MiniMax-M3** (via built-in `minimax-cn` provider → Anthropic-compat `api.minimaxi.com/anthropic`, China platform, key mirrored to `MINIMAX_CN_API_KEY`; Agnes `agnes-2.0-flash` was too slow); BOTH subagents off deepseek-v4-flash → **MiniMax-M2.7-highspeed** (Hermes `delegation` in config.yaml; finance analyst via a new `minimax` provider in `llm.py` + `FINANCE_LLM_PROVIDER=minimax`, `max_tokens` 300→800 for the reasoning model's `<think>`; verified `llm:MiniMax-M2.7-highspeed` appears in `/v1/analyze`). Container rebuilt/recreated + finance restarted; M3 ~1.0s. **Deeper research ingestion (P0.75-A)**: `research_ingest.py` (fundamentals + earnings-calendar docs → knowledge store, provenance-mandatory, deduped, fail-closed) wired into `on_monitor` so RAG cites real substance beyond news (test: `search_knowledge` retrieves an ingested fundamentals doc). 7 new tests; suite 741. NEXT: P0.8 resilience.
- 2026-07-13 — **Phase roadmap 0.8/0.9/0.95 defined + earnings calendar built** (human decision: use the ~2-week IBKR-account wait to harden the backbone; "make the brain smarter"). Added §7 phases: **0.8 Resilience & observability** (feed/loop resilience, health/heartbeat + dead-man's switch, ledger↔broker reconciliation), **0.9 Broker-integration backbone** (IBKRBroker via ib_async built + mock-tested OFFLINE — bracket/OCA, client-order-id idempotency, T+1 settled-cash, partial/reject/cancel, reconciliation, paper↔live gap harness), **0.95 Pre-live validation gate ⛔** (upstream sync merge, IBKR-paper dry run, walk-forward ≥2 regimes, go-live runbook + kill-switch, human sign-off). This front-loads the Phase-1 code during the account wait. P0.75 brain progress: **earnings/events calendar** (`earnings.py` YFinanceEarnings — injectable, cached, fail-None; `upcoming_earnings`) wired into `build_research_brief(earnings=...)` → EventsView + "earnings imminent" warning; the "earnings not wired" unknown clears. 6 tests; suite 728.
- 2026-07-13 — **Phase 0.75-B3 (K-line skill) + dual-bot behavior refinement** (human directive: "apply k-line chart in chat as a skill, only render for a specific stock in detail to save tokens; general bot replies in group, finance bot only sends confirmations + replies on @mention/DM; does hermes support 2 telegram bots concurrently?"). DONE + deployed + live-verified: (1) **K-line chart Skill** `skills/finance/kline-chart/` — frontmatter `description` gates it to detailed SINGLE-stock requests (multi-symbol / one-line price checks / macro stay text-only); `scripts/render_kline.py` fetches `/v1/bars` itself and renders a candlestick PNG with Pillow (present in the runtime; zero new deps), printing ONLY `{chart,summary}` so bars never enter the transcript; agent replies summary + `MEDIA:<png>` (rendered on every chat surface). Env-agnostic script path `${HERMES_HOME:-$HOME/.hermes}/skills/...`. Verified live in-container for NVDA + 0700.HK. `requires_toolsets:[finance]` gates visibility. (2) **Finance-bot @mention/DM replies**: `TelegramSurfaceAdapter` now handles text — replies ONLY on a DM or an @mention of the bot (allowlist-gated, mention stripped), otherwise stays quiet; a ticker → quick multi-agent read via the shared `on_demand.analyze_symbol` (also now backs `/v1/analyze`, DRY); `HttpTransport.get_me()` added for @mention detection. (3) 2-bot answer: two bots run as two processes (gateway=general token, finance service=finance token); one token=one long-poller. 18 new trader tests; suite 722. Skills map: skills are `SKILL.md` dirs under `${HERMES_HOME}/skills`, model-auto-selected by description, images via `MEDIA:` tag.
- 2026-07-13 — **Phase 0.75 started — deepen the brain + conversational finance agent** (human picked "deepen the analysis brain", then clarified: "we want the current hermes bot (the general one) to handle complex finance analysis and give feedback on the real-time market (k-chart, current stock price)"). §4b-style contract added to §7. Delivered + DEPLOYED + live-verified: (A) real fundamentals — `fundamentals.YFinanceFundamentals` (yfinance .info: P/E, fwd P/E, revenue growth, margins; injectable factory, TTL cache incl. negatives, fail-None) wired into the US loop's FundamentalAgent (was empty). (B) on-demand READ-ONLY endpoints `/v1/quote`, `/v1/bars` (K-line), `/v1/analyze` (technical+fundamental+sentiment+debate+LLM) + FinanceRuntime feed/fundamentals/llm_analyst. (B) **Finance toolset for the general Hermes agent** — `tools/finance_tools.py` (get_quote/get_kline/analyze_symbol/research_brief/search_research/account_view; thin httpx wrappers over the service API, never trader internals; toolset `finance` in `_HERMES_CORE_TOOLS`, auto-enables on `HERMES_FINANCE_SERVICE_URL`, which the gateway compose now sets). §3/§8 HARD: NO order/approve tool (test-asserted). Verified LIVE IN THE CONTAINER: get_quote NVDA→210.96; analyze_symbol NVDA→"long 0.52" from technical+fundamental+sentiment+llm:deepseek-v4-flash. Trader suite 713; 15 new trader tests + 9 new root tool tests; the only failing root tests (TestParallelClientConfig) are pre-existing/unrelated. REMAINING P0.75: earnings calendar, LLM decision core (RAG), feedback loop, deeper ingestion; and B3 K-line chart rendering in web/desktop chat.
- 2026-07-13 — **Two-session extension (CN morning research + US evening) & dual-bot roles started** (human directive; §4b added as standing contract). Backend DONE + 700 trader tests green (RiskEngine still 100% branch cov): (1) dual-bot split — reporter = shared gateway bot outbound-only (daily summaries/briefs), gatekeeper = dedicated finance bot interactive approval-only (own token, allowlist); `dailyloop.on_push` sends preamble via reporter + cards via gatekeeper; `__main__` builds two transports. (2) `scheduler.SessionSchedule` generalizes US+CN (Asia/Shanghai, mainland+HKEX 2026 holidays) with ZERO change to US behaviour (77 US scheduler tests untouched). (3) `research_session.ResearchSession` = CN morning research-ONLY loop (monitors+technical/sentiment/debate+optional LLM → lighter brief; NO decision core/risk-exec/confirmation/broker → structurally cannot order); `cn_watchlist` mainland A-share+HK tech-focused, config-editable (`FINANCE_CN_SYMBOLS`), graceful HK-only degrade. (4) `build_research_brief` gained `watchlist_lookup`/`trading_tz`/`signals`/`candidates`/`include_account` so CN research never pollutes the US ledger; CN brief at `/v1/research/brief?market=cn`; `brief_telegram` renderer (zh default/en). (5) search/summary LLM pinned to `deepseek-v4-flash` (`FINANCE_LLM_SEARCH_MODEL`, role-based) to save token fees. 12 new tests (`test_cn_session.py`). UI pass DONE (parallel agents): Web migrated ~63 hard-coded finance strings (ApprovalQueue.tsx + HistorySection.tsx were 100% un-i18n'd English) to en+zh + added the US/China·HK market toggle (research brief stays primary/default US; CN mode hides RiskStrip/approval-queue/account/orders since risk is null + report-only, shows a research-only badge); Desktop migrated ~38 (hard-coded sidebar Finance nav label + a `finance.enums` catalog localizing all backend enum vocabularies in pills/tables) to en+zh + the same toggle. Web typecheck/lint/build + Desktop typecheck/lint all green; trader suite 701 (added on_send empty-push guard: a mid-day restart after the CN events passed must not push a contentless brief). **DEPLOYED**: gateway docker image rebuilt (web bundle `hermes_cli/web_dist` verified to contain the CN toggle + zh strings) and container recreated (dashboard 9119 → 200); host launchd finance service (9319) restarted — logs confirm reporter bot (shared, outbound-only) + gatekeeper bot (dedicated, interactive, 1 allowlisted user) + CN research session (21 symbols) + LLM `deepseek-v4-flash`. Also repaired `scripts/hermes_daily_redeploy.sh` (referenced a removed `dashboard` service → `gateway`) so the daily auto-redeploy works again.
...
- 2026-07-13 — P0.5 backlog#3/#4 done + #5 complete: research-first UI shipped on BOTH surfaces. Web: ResearchBrief section renders FIRST (as-of/PAPER-LIVE header, staleness banner, risk strip w/ TRIPPED banner, regime, movers w/ theme-role tags, themes, sentiment-tinted sourced news digest, debate-first signals, full uncertainty box, provenance footer, debounced knowledge search w/ calm 503-offline note); Queue collapsed to a badged "Actions requiring attention" strip (hidden when empty). Desktop: Investment Research is the DEFAULT tab; Action queue = secondary tab with pending-count badge; portfolio sub-tabs. i18n MANDATE MET: web finance catalog (~130 keys, en+zh; nav key in all 16 locales), desktop finance section (~170 keys, en+zh) incl. migration of ALL Phase-0 hard-coded strings. All checks green (web typecheck/lint-scoped/test/build; desktop typecheck/lint-scoped/vite build; trader suite 687). Phase 0.5 now blocked ONLY on the human-created dedicated Telegram bot.
- 2026-07-13 — P0.5 backlog#2 done + #5 backend half done: `brief.py` ResearchBrief (per-source freshness w/ 120-min staleness + human warnings; regime/risk incl. ledger fallback; movers by dist-SMA20 w/ theme/role tags; theme aggregation; top-10 |sentiment| news digest; today's debate-first signals + candidate counts; EXPLICIT unknowns — earnings feed missing, fundamentals empty, LLM on/off; provenance links; degraded all-None mode never raises; 28 tests). `knowledge_pipeline.py` (build_knowledge fail-closed → (knowledge, None); ingest_news_snapshot: provenance-mandatory docs + append-only facts + vector indexing, dupes counted not re-indexed; search_knowledge raises KnowledgeUnavailable when down; 16 tests). Wired: daily loop ingests news + publishes the brief at monitor/decide; API GET /v1/research/brief (on-demand degraded fallback) + GET /v1/knowledge/search (503 fail-closed); serve builds the store under trader/data/knowledge (FINANCE_QDRANT_URL switches to the container). Suite 687 green; service restarted, brief endpoint live.

...
#: Telegram REQUIRES command names be lowercase [a-z0-9_]. Descriptions are in
#: English (matching the general Hermes bot). The handler also accepts the
#: Chinese aliases (/持仓 …) typed by hand.
COMMAND_MENU: list[tuple[str, str]] = [
    ("holdings", "Your real holdings and cost basis"),
    ("brief", "Market research brief (choose CN or KR)"),
    ("record", "How to record a trade (examples)"),
...
_BRIEF_REGIONS = {"cn": "中港", "kr": "韩国半导体"}


def _fmt_brief(runtime: Any, region: Optional[str] = None) -> str:
    # Ask which region first (user directive): /brief cn or /brief kr.
    if region not in _BRIEF_REGIONS:
        return ("研究哪个地区？\n"
                "• /brief cn — 中港\n"
                "• /brief kr — 韩国半导体")
    label = _BRIEF_REGIONS[region]
    b = (getattr(runtime, "latest_briefs", {}) or {}).get(region)
    if not b:
        return f"{label}今日还没有研究简报（定时任务或 Research 面板刷新后再看）。"
    movers = (b.get("movers") or {}).get("top", []) if isinstance(b.get("movers"), dict) else []
    out = [f"【{label}】as-of {str(b.get('as_of', ''))[:10]}"]
    for m in movers[:6]:

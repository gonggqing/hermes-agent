...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Lifecycle management applies only to topics Hermes created itself.
        forum_topics_config = extra.get("forum_topics", {})
        if not isinstance(forum_topics_config, dict):
            forum_topics_config = {}
        self._forum_topics_enabled = bool(forum_topics_config.get("enabled", False))
        try:
            archive_check_hours = float(forum_topics_config.get("archive_check_hours", 6))
        except (TypeError, ValueError):
            archive_check_hours = 6.0
        self._forum_archive_check_seconds = max(1.0, min(24.0, archive_check_hours)) * 3600.0
    ...
    def _start_forum_archive_task(self) -> None:
        """Start the idle-archive loop for Hermes-managed group topics."""
        if not self._forum_topics_enabled or not self._bot:
            return
        task = self._forum_archive_task
        if task and not task.done():
            return
        self._forum_archive_task = asyncio.ensure_future(self._run_forum_archive_loop())
        ...
        try:
            while True:
                if self._bot is not None:
                    ...
                    result = await archive_inactive_topics(self._bot)
                    if result["archived"] or result["failed"]:
                        logger.info(
                            "[%s] Forum auto-archive: %d archived, %d failed",
                            self.name, result["archived"], result["failed"],
                        )
                await asyncio.sleep(self._forum_archive_check_seconds)
        except asyncio.CancelledError:
            raise
        except Exception:
            logger.warning(
    ...
    def _touch_managed_forum_topic(self, message: Message) -> None:
        """Record authorized activity for a Hermes-managed group topic."""
        if not self._forum_topics_enabled:
            return
        chat = getattr(message, "chat", None)
        thread_id = getattr(message, "message_thread_id", None)
        if chat is None or getattr(chat, "type", None) not in {"group", "supergroup"} or thread_id in {None, 1, "1"}:
            return
        try:
            ...
            logger.debug("[%s] Could not persist forum-topic activity", self.name, exc_info=True)
...
def _apply_yaml_config(yaml_cfg: dict, telegram_cfg: dict) -> dict | None:
    ...
    if isinstance(telegram_cfg.get("forum_topics"), dict):
        # Copy so adapter-local normalization can never mutate config.yaml's
        # parsed object shared with another profile or platform instance.

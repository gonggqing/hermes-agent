...
export const en: Translations = {
  ...
  finance: {
    ...
    breakerPill: state => `breaker ${state}`,
    loopAttached: 'loop attached',
    loopIdle: 'loop idle',
    asOf: ts => `as of ${ts}`,
    connecting: 'Connecting to finance service…',
    breakerBannerTitle: 'Circuit breaker TRIPPED — no new entries today.',
    breakerBannerBody:
      'The −4% daily drawdown guardrail halted new entries (Loop.md §3). Exits and resting protection remain active. The breaker resets with the next trading day.',
    offlineTitle: 'Finance service offline',
    offlineHint:
      'The swing-trader service is not running. Start it with: cd trader && uv run python -m swing_trader serve',
    offlineRetry: 'Retry now',
    ...
    queue: {
      ...
      edit: 'Edit',
      reject: 'Reject',
      confidenceVersion: (confidence, version) => `confidence ${confidence} · v${version}`,
      qty: 'Qty',
      limit: 'Limit',
      stop: 'Stop',
      tp: 'TP',
      ...
      editTitleFor: symbol => `Edit candidate · ${symbol}`,
      editDescription:
        'Adjust quantity and prices, then approve. The risk engine re-validates every edit before submission — protection can never be removed.',
      fieldQty: 'Quantity',
      fieldLimit: 'Limit',
      fieldStop: 'Stop',
    ...
    account: {
      ...
      drawdown: 'Drawdown',
      breaker: 'Breaker',
      sourceLedger: ts => `ledger · ${ts}`,
      sourceLive: ts => `live · ${ts}`,
      positionsTitle: 'Positions',
      positionsLoopIdle: 'Positions are only visible while the trading loop is attached.',
      positionsEmpty: 'No open positions.',
      ordersTitle: 'Open orders',
      ordersEmpty: 'No working orders.',
      colSymbol: 'Symbol',
      colQty: 'Qty',
      colAvgPx: 'Avg px',
      colMktPx: 'Mkt px',
      ...
      colStatus: 'Status',
      colPlaced: 'Placed',
      statsTitle: 'Trade stats (closed trades)',
      statClosedWins: 'Closed / wins',
      statWinRate: 'Win rate',
      statAvgWinLoss: 'Avg win / loss',
      statPayoff: 'Payoff ratio',
      statExpectancy: 'Expectancy',
      statTotalPnl: 'Total PnL',
      statAvgHold: 'Avg hold (days)',
      statMaxDrawdown: 'Max drawdown',
      equityAlt: 'Equity history',
      equitySpark: count => `Equity, last ${count} snapshots`
    },
    market: {
      ...
      asOfLabel: 'As of',
      vs50dma: delta => `${delta} vs 50DMA`,
      watchlistTitle: 'Watchlist universe',
      watchlistNote: '(monitored set, not a buy list)',
      watchlistEmptySearch: 'No watchlist entries match the search.',
      watchlistEmpty: 'Watchlist unavailable.',
      disabledTag: '(off)'
    },
    history: {
      ...
      empty: 'No candidates recorded for this mode yet.',
      emptySearch: 'No candidates match the search.',
      rowConfidence: pct => `confidence ${pct}`,
      auditTitle: (symbol, id) => `Audit trail · ${symbol} (${id}…)`,
      auditEmpty: 'No audit events for this candidate.',
      auditEvent: (actor, surface, version) => `by ${actor} via ${surface} (v${version})`,
      applied: 'applied',
    ...
    watch: {
      ...
      delayNote: 'Prices and analysis may be delayed and are indicative only — not execution quotes.',
      ...
      custom: {
        ...
        add: 'Add instrument',
        searchPlaceholder: 'Search listed stock or ETF',
        heldSuggestions: 'From your holdings',
        noMatches: 'No listed stock or ETF found. Try a ticker, name, or market suffix.',
        rename: 'Rename',
        delete: 'Delete',
      ...
      analyzing: 'Analyzing…',
      analyzeError: 'Analysis unavailable for this symbol.',
      verdictTitle: 'Verdict',
      verdictLine: (direction, confidence) => `${direction} · confidence ${confidence}`,
      signalsTitle: 'Agent signals',
      signalsEmpty: 'No agent signals returned.',
      signalConfidence: pct => `confidence ${pct}`,
      newsTitle: 'News',
      researchTitle: 'Research',
      citationsEmpty: 'No sources cited.'
    },
    portfolio: {
      ...
      history: 'History',
      reports: 'Reports',
      selectPrompt: 'Select a position or an overview section.',
      positionQty: 'Quantity',
      positionAvgPx: 'Avg price',
    ...
    holdings: {
      ...
      subnavPaper: 'Paper account',
      subnavReal: 'My holdings',
      paperAccountName: 'IBHK Paper',
      ...
      accountsEmpty: 'No accounts in this view yet. Add a US, HK, or CN account to start tracking holdings.',
      accountsLoadError: 'Could not load accounts.',
      selectPrompt: 'Select an account, the aggregate view, or drafts.',
      readOnlyNote: 'Read-only — the only write is confirming a draft you review. No orders are placed here.',
      tabsAria: 'Account view',
      ...
      heldInAccounts: n => `${n} accounts`,
      aggregateTitle: 'All accounts',
      aggregateEmpty: 'No holdings across any account yet.',
      ...
      colSource: 'Source',
      colNote: 'Note',
      activityEmpty: 'No activity recorded for this account.',
      reconcileTitle: 'Reconciliation',
      reconcileOk: 'In sync',
      reconcileDrift: 'Drift detected',
      reconcileAuthority: authority => `authority: ${authority}`,
      driftTitle: 'Drift',
      colPortfolioQty: 'Tracked qty',
      colBrokerQty: 'Broker qty',
      noDrift: 'No drift — tracked holdings match the authority.',
      reconcileEmpty: 'No reconciliation data for this account.',
      addAccountTitle: 'Add account',
      addAccountDesc: 'Register a real brokerage account. Holdings are derived from the events you record or import.',
      editAccountTitle: name => `Edit account · ${name}`,
      editAccountDesc: 'Update the account name, risk inclusion, type, or note.',
      fieldName: 'Name',
      ...
      fieldEnvironment: 'Account environment',
      fieldIncludeInRisk: 'Include in risk',
      includeInRiskHint: 'Count this account toward portfolio risk aggregates.',
      fieldNote: 'Note',
      notePlaceholder: 'Optional',
      createAccount: 'Create account',
      creating: 'Creating…',
      saveAccount: 'Save',
      accountCreated: name => `Account “${name}” created.`,
      accountUpdated: name => `Account “${name}” updated.`,
      accountSaveFailed: 'Could not save the account.',
      nameRequired: 'Enter an account name.',
      recordTradeTitle: account => `Record trade · ${account}`,
      recordTradeDesc:
        'Recording creates a draft and confirms it as you (desktop) so it lands as a holding. Leave price blank if the cost is unknown.',
      fieldInstrument: 'Instrument',
      instrumentPlaceholder: 'Search symbol or name…',
      instrumentSearchPlaceholder: 'Type at least 2 characters…',
      instrumentMinChars: 'Type at least 2 characters to search.',
      instrumentSearching: 'Searching…',
      instrumentEmpty: q => `No instruments match “${q}”.`,
      instrumentDegraded: 'Resolver degraded — showing cached matches.',
      instrumentPicked: (symbol, market, currency) => `${symbol} · ${market} · ${currency}`,
      fieldEventType: 'Event',
      fieldQty: 'Quantity',
      fieldPrice: 'Price',
      priceUnknownHint: 'Blank = unknown cost',
      fieldCommission: 'Commission',
      fieldOccurredAt: 'Occurred at',
      submitTrade: 'Record',
      submitting: 'Recording…',
      tradeRecorded: symbol => `Recorded ${symbol}.`,
      tradeFailed: 'Could not record the trade.',
      instrumentRequired: 'Select an instrument.',
      qtyRequired: 'Enter a quantity.',
      positiveNumber: label => `${label} must be a positive number.`,
      draftsTitle: 'Drafts to review',
      draftsEmpty: 'No drafts to review. Drafts Hermes creates from chat appear here for you to confirm.',
      draftMissing: fields => `Missing: ${fields}`,
      draftAmbiguities: items => `Ambiguous: ${items}`,
      draftOriginal: text => `“${text}”`,
      draftCreatedBy: actor => `by ${actor}`,
      confirm: 'Confirm',
      edit: 'Edit',
      reject: 'Reject',
      draftConfirmed: symbol => `Confirmed ${symbol}.`,
      draftRejected: symbol => `Rejected ${symbol}.`,
      draftActionDone: (symbol, action, code) => `${symbol}: ${action} ${code}`,
      draftActionFailed: (action, symbol) => `Failed to ${action} ${symbol}`,
      draftRowMeta: (event, status) => `${event} · ${status}`,
      editDraftTitle: 'Edit & confirm draft',
      editDraftDesc: 'Fix quantity, price, or commission, then confirm. The service re-validates every edit.',
      saveConfirm: 'Save & confirm',
      importTitle: account => `Import CSV · ${account}`,
      importDesc:
        'Paste rows, preview, then commit. Columns: date, event_type, symbol, market, currency, qty, price, commission, amount, external_id, note (date + event_type required).',
      csvLabel: 'CSV',
      csvPlaceholder: 'date,event_type,symbol,market,currency,qty,price,commission,amount,external_id,note',
      csvColumnsHint: 'date and event_type are required on every row.',
      preview: 'Preview',
      previewing: 'Previewing…',
      commit: 'Commit',
      committing: 'Committing…',
      importHeaderError: error => `Header error: ${error}`,
      importCounts: (valid, invalid, duplicate) => `${valid} valid · ${invalid} invalid · ${duplicate} duplicate`,
      importNotCommittable: 'Fix the invalid rows before committing.',
      importColLine: 'Line',
      importColStatus: 'Status',
      importColEvent: 'Event',

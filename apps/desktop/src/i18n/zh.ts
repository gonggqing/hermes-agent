...
export const zh: Translations = {
  ...
  finance: {
    ...
    modePaper: '模拟',
    modeLive: '实盘',
    searchPlaceholder: '按代码或主题筛选',
    serviceOnline: '服务在线',
    serviceOffline: '服务离线',
    serviceConnecting: '连接中…',
    breakerPill: state => `熔断 ${state}`,
    loopAttached: '交易循环已连接',
    loopIdle: '交易循环空闲',
    asOf: ts => `截至 ${ts}`,
    connecting: '正在连接金融服务…',
    breakerBannerTitle: '熔断器已触发（TRIPPED）— 今日停止新开仓。',
    breakerBannerBody:
      '−4% 单日回撤保护已停止新开仓（Loop.md §3）。平仓与既有保护单仍然有效。熔断器将在下一个交易日重置。',
    offlineTitle: '金融服务离线',
    offlineHint: 'swing-trader 服务未运行。启动方式：cd trader && uv run python -m swing_trader serve',
    offlineRetry: '立即重试',
    offlineChecking: '检查中…',
    modeBarAria: '交易模式',
    modeFollowsService: '跟随服务',
    modeOverridden: '手动覆盖',
    ...
    queue: {
      ...
      edit: '编辑',
      reject: '拒绝',
      confidenceVersion: (confidence, version) => `置信度 ${confidence} · v${version}`,
      qty: '数量',
      limit: '限价',
      stop: '止损价',
      tp: '止盈',
      ...
      editTitle: '编辑候选单',
      editTitleFor: symbol => `编辑候选单 · ${symbol}`,
      editDescription: '调整数量与价格后批准。风控引擎会在提交前重新校验每一次编辑 — 保护单永远不会被移除。',
      fieldQty: '数量',
      fieldLimit: '限价',
      fieldStop: '止损价',
      fieldSl: '止损（SL）',
    ...
    account: {
      ...
      drawdown: '回撤',
      breaker: '熔断器',
      sourceLedger: ts => `账本 · ${ts}`,
      sourceLive: ts => `实时 · ${ts}`,
      positionsTitle: '持仓',
      positionsLoopIdle: '仅在交易循环连接时才能查看持仓。',
      positionsEmpty: '暂无持仓。',
      ordersTitle: '未成交订单',
      ordersEmpty: '暂无挂单。',
      colSymbol: '代码',
      ...
      colPlaced: '下单时间',
      statsTitle: '交易统计（已平仓）',
      statClosedWins: '平仓 / 获胜',
      statWinRate: '胜率',
      statAvgWinLoss: '平均盈 / 亏',
      statPayoff: '盈亏比',
      statExpectancy: '期望值',
      statTotalPnl: '总盈亏',
      statAvgHold: '平均持仓（天）',
      statMaxDrawdown: '最大回撤',
      equityAlt: '净值历史',
      equitySpark: count => `净值，最近 ${count} 个快照`
    },
    market: {
      regimeTitle: '市场状态',
      ...
      breadth: '广度 > 50 日均线',
      asOfLabel: '截至',
      vs50dma: delta => `${delta} 相对 50 日均线`,
      watchlistTitle: '观察池',
      watchlistNote: '（监控范围，非买入清单）',
      watchlistEmptySearch: '没有匹配搜索的观察池条目。',
      watchlistEmpty: '观察池不可用。',
      disabledTag: '（停用）'
    },
    history: {
      title: '候选单历史',
      ...
      empty: '该模式下尚未记录候选单。',
      emptySearch: '没有匹配搜索的候选单。',
      rowConfidence: pct => `置信度 ${pct}`,
      auditTitle: (symbol, id) => `审计记录 · ${symbol}（${id}…）`,
      auditEmpty: '该候选单暂无审计事件。',
      auditEvent: (actor, surface, version) => `由 ${actor} 通过 ${surface} 操作（v${version}）`,
      applied: '已生效',
      notApplied: '未生效'
    },
    reports: {
      empty: '暂无报告 — 报告器在每个交易日（美东 09:00）发布晨报。',
      reportKind: kind => `${kind} 报告`
    },
    research: {
      tradingDay: date => `交易日 ${date}`,
      modePaper: 'PAPER · 模拟盘',
      modeLive: 'LIVE · 实盘',
      briefAsOf: ts => `简报截至 ${ts}`,
      briefError: '无法加载投资研究简报。',
      staleTitle: '数据新鲜度警告',
      narrativeTitle: '研究员简报',
      narrativeSubtitle: '用容易理解的方式说明今天的数据意味着什么、为什么值得关注，以及哪些地方仍需核实。',
      narrativeMarketLabel: '市场判断',
      narrativeFocusLabel: '当前主线',
      narrativeOpportunityLabel: '值得继续研究',
      narrativeRiskLabel: '风险与下一步',
      narrativeMarketRiskOn: '风险偏好较好，但这不代表所有标的都适合买入。',
      narrativeMarketNeutral: '市场信号分化，更适合精选标的，而不是笼统增加整体仓位。',
      narrativeMarketRiskOff: '风险偏好偏弱，当前应优先保护本金，并提高入场要求。',
      narrativeMarketUnknown: '当前市场数据不足，无法可靠判断市场环境。',
      narrativeMarketFacts: (vix, breadth) => `VIX 为 ${vix}；观察范围中有 ${breadth}% 位于 50 日均线上方。`,
      narrativeThemeFocus: (theme, distance, leaders) =>
        `${theme} 是当前最强主题（相对 50 日均线 ${distance}），领先标的包括 ${leaders}。`,
      narrativeMoverFocus: (symbol, distance) =>
        `${symbol} 是当前最明显的异动标的（相对 20 日均线 ${distance}）。`,
      narrativeFocusUnknown: '目前还没有足够完整的主题或异动快照。',
      narrativeOpportunity: (symbol, score, reasons) =>
        `${symbol} 以 ${score} 分位列发现池第一。主要原因：${reasons}。`,
      narrativeOpportunityNone: '今天没有新标的同时通过证据、趋势和流动性检查。',
      narrativeRiskClear: '本次快照没有额外的监控警告。',
      narrativeRiskSummary: (freshness, risk, unknown) =>
        `当前有 ${freshness} 项数据新鲜度警告、${risk} 项组合风险提示和 ${unknown} 项研究缺口需要核实。`,
      narrativeAction: '发现评分只表示研究优先级，不代表预期收益或买入评级；下单前仍需核实证据、估值和组合风险。',
      freshMarket: '市场',
      freshNews: '新闻',
      freshPortfolio: '组合',
      freshMissing: '缺失',
      freshAge: minutes => `${minutes} 分钟前`,
      riskTitle: '风险',
      riskDescription: '回答组合是否承受得起新增风险。熔断、回撤、现金和警告应优先于任何看起来有吸引力的机会。',
      riskEmpty: '暂无风险数据 — 风险监控和账本快照都尚不可用。',
      poolExposure: '资金池敞口',
      regimeDescription: '综合波动率、市场广度和主要指数趋势，判断当前环境更偏进攻、中性还是防守。',
      moversTitle: '观察池异动',
      moversDescription: '按相对 20/50 日均线的异常偏离排序。偏离较大只表示需要关注，不等于便宜或值得追涨。',
      moversTop: '领涨（相对 SMA20）',
      moversBottom: '领跌（相对 SMA20）',
      moversEmpty: '暂无异动数据 — 组合监控尚未生成观察快照。',
      vsSma20: delta => `${delta} 相对 SMA20`,
      vsSma50: delta => `${delta} 相对 SMA50`,
      themesTitle: '主题',
      themesDescription: '把相关标的合并观察，用来判断强弱是否具有行业或产业链共性，而不是单一个股的偶然波动。',
      themesEmpty: '暂无主题数据。',
      discoveryTitle: '新机会发现池',
      discoveryDescription: '仅供继续研究的候选标的，已通过来源、代码、趋势和流动性检查，但尚未通过组合风控与人工确认。',
      discoveryEmpty: '今日暂无通过来源、代码、趋势和流动性验证的新标的。',
      discoveryScore: score => `研究评分 ${score}`,
      discoveryScoreMeaning: '评分根据已保留的证据和筛选因子排列研究优先级，不是收益预测，也不是买入评级。',
      discoveryReasons: '评分依据',
      discoveryNoReasons: '该标的没有保留下更细的评分原因。',
      discoveryReasonTrend: value => `20 日价格趋势 ${value}`,
      discoveryReasonRelativeStrength: value => `相对强度 ${value}`,
      discoveryReasonVolume: value => `成交量为 20 日均值的 ${value}`,
      discoverySource: source => `证据：${source}`,
      synthesisTitle: '中港跨市场综合',
      synthesisEmpty: '等待 A 股与港股两份独立研报完成。',
      themeMeta: (count, avg) => `${count} 个标的 · 平均 ${avg} 相对 SMA50`,
      themeLeaders: symbols => `领先：${symbols}`,
      newsTitle: '新闻摘要',
      newsDescription: '突出最重要且有来源的新闻。情绪分数只帮助排序，调整判断前仍应阅读原始来源。',
      newsEmpty: '最新快照中没有新闻条目。',
      sentiment: value => `情绪 ${value}`,
      signalsTitle: '今日信号',
      signalsDescription: '汇总各分析 Agent 的方向、置信度和核心论点。信号仍属于研究，必须经过风控与确认才可能执行。',
      ...
      openQueue: '打开操作队列',
      uncertaintyTitle: '不确定性与未知项',
      uncertaintyDescription: '列出缺失、过期或尚未解决的证据；这些是决策约束，不是可以略过的脚注。',
      provenanceTitle: '来源',
      searchTitle: '研究检索',
      searchPlaceholder: '搜索已收集的研究资料（新闻、公告、笔记）…',
      searchRun: '搜索',
    ...
    watch: {
      ...
      delayNote: '价格与分析可能有延迟，仅供参考，并非可执行报价。',
      readOnlyTag: '只读',
      modules: {
        gold: '黄金',
        oil: '原油',
      ...
      custom: {
        ...
        add: '添加标的',
        searchPlaceholder: '搜索场内股票或 ETF',
        heldSuggestions: '从已有持仓添加',
        noMatches: '没有匹配的场内股票或 ETF，请尝试代码、名称或市场后缀。',
        rename: '重命名',
        delete: '删除',
        remove: '从自选组移除',
      ...
      hideAnalysis: '隐藏分析',
      analyzing: '分析中…',
      analyzeError: '该标的分析不可用。',
      verdictTitle: '结论',
      verdictLine: (direction, confidence) => `${direction} · 置信度 ${confidence}`,
      signalsTitle: '智能体信号',
      signalsEmpty: '未返回智能体信号。',
      signalConfidence: pct => `置信度 ${pct}`,
      newsTitle: '新闻',
      researchTitle: '研究',
      citationsEmpty: '暂无引用来源。'
    },
    portfolio: {
      groupOverview: '总览',
      groupPositions: '持仓',
    ...
    holdings: {
      ...
      subnavPaper: '模拟账户',
      subnavReal: '我的持仓',
      paperAccountName: 'IBHK Paper（盈透模拟账户）',
      environmentLive: '实盘',
      environmentPaper: '模拟盘',
      groupPortfolio: '组合',
      groupAccounts: '账户',
      aggregate: '全部账户',
      drafts: '草稿',
      addAccount: '添加账户',
      accountMeta: (market, type) => `${market} · ${type}`,
      accountsEmpty: '当前视图暂无账户。添加美股、港股或 A 股账户以开始跟踪持仓。',
      accountsLoadError: '无法加载账户。',
      selectPrompt: '选择一个账户、汇总视图或草稿。',
      readOnlyNote: '只读 — 唯一的写入是确认你审核过的草稿。此处不下任何订单。',
      tabsAria: '账户视图',
      tabHoldings: '持仓',
      tabActivity: '流水',
      tabReconcile: '对账',
      ...
      cashTitle: '现金',
      cashEmpty: '暂无现金余额记录。',
      heldInAccounts: n => `${n} 个账户`,
      aggregateTitle: '全部账户',
      aggregateEmpty: '所有账户均暂无持仓。',
      riskOnly: '仅计入风险的账户',
      ...
      colSource: '来源',
      colNote: '备注',
      activityEmpty: '该账户暂无流水记录。',
      reconcileTitle: '对账',
      reconcileOk: '一致',
      reconcileDrift: '发现偏差',
      reconcileAuthority: authority => `以 ${authority} 为准`,
      driftTitle: '偏差',
      colPortfolioQty: '记录数量',
      colBrokerQty: '券商数量',
      noDrift: '无偏差 — 记录持仓与权威来源一致。',
      reconcileEmpty: '该账户暂无对账数据。',
      addAccountTitle: '添加账户',
      addAccountDesc: '登记一个真实券商账户。持仓由你记录或导入的事件推导得出。',
      editAccountTitle: name => `编辑账户 · ${name}`,
      editAccountDesc: '更新账户名称、风险计入、类型或备注。',
      fieldName: '名称',
      namePlaceholder: '例如 盈透主账户',
      fieldMarket: '市场',
      fieldBaseCurrency: '基础币种',
      fieldProvider: '券商',
      fieldAccountType: '账户类型',
      fieldEnvironment: '实盘 / 模拟盘',
      fieldIncludeInRisk: '计入风险',
      includeInRiskHint: '在组合风险汇总中计入该账户。',
      fieldNote: '备注',
      notePlaceholder: '可选',
      createAccount: '创建账户',
      creating: '创建中…',
      saveAccount: '保存',
      accountCreated: name => `账户“${name}”已创建。`,
      accountUpdated: name => `账户“${name}”已更新。`,
      accountSaveFailed: '无法保存账户。',
      nameRequired: '请输入账户名称。',
      recordTradeTitle: account => `记录交易 · ${account}`,
      recordTradeDesc: '记录会创建草稿并以你的身份（桌面端）确认，使其成为持仓。若成本未知可留空价格。',
      fieldInstrument: '标的',
      instrumentPlaceholder: '搜索代码或名称…',
      instrumentSearchPlaceholder: '至少输入 2 个字符…',
      instrumentMinChars: '至少输入 2 个字符以搜索。',
      instrumentSearching: '搜索中…',
      instrumentEmpty: q => `没有匹配“${q}”的标的。`,
      instrumentDegraded: '解析降级 — 显示缓存的匹配结果。',
      instrumentPicked: (symbol, market, currency) => `${symbol} · ${market} · ${currency}`,
      fieldEventType: '事件',
      fieldQty: '数量',
      fieldPrice: '价格',
      priceUnknownHint: '留空 = 成本未知',
      fieldCommission: '佣金',
      fieldOccurredAt: '发生时间',
      submitTrade: '记录',
      submitting: '记录中…',
      tradeRecorded: symbol => `已记录 ${symbol}。`,
      tradeFailed: '无法记录交易。',
      instrumentRequired: '请选择一个标的。',
      qtyRequired: '请输入数量。',
      positiveNumber: label => `${label} 必须为正数。`,
      draftsTitle: '待审核草稿',
      draftsEmpty: '暂无待审核草稿。Hermes 从对话中创建的草稿会显示在此处等待你确认。',
      draftMissing: fields => `缺失：${fields}`,
      draftAmbiguities: items => `含糊：${items}`,
      draftOriginal: text => `“${text}”`,
      draftCreatedBy: actor => `来自 ${actor}`,
      confirm: '确认',
      edit: '编辑',
      reject: '拒绝',
      draftConfirmed: symbol => `已确认 ${symbol}。`,
      draftRejected: symbol => `已拒绝 ${symbol}。`,
      draftActionDone: (symbol, action, code) => `${symbol}：${action} ${code}`,
      draftActionFailed: (action, symbol) => `${symbol} ${action}失败`,
      draftRowMeta: (event, status) => `${event} · ${status}`,
      editDraftTitle: '编辑并确认草稿',
      editDraftDesc: '修正数量、价格或佣金后确认。服务会对每次编辑重新校验。',
      saveConfirm: '保存并确认',
      importTitle: account => `导入 CSV · ${account}`,
      importDesc:
        '粘贴数据行，预览后提交。列：date、event_type、symbol、market、currency、qty、price、commission、amount、external_id、note（date 与 event_type 必填）。',
      csvLabel: 'CSV',
      csvPlaceholder: 'date,event_type,symbol,market,currency,qty,price,commission,amount,external_id,note',
      csvColumnsHint: '每行的 date 与 event_type 为必填。',
      preview: '预览',
      previewing: '预览中…',
      commit: '提交',
      committing: '提交中…',
      importHeaderError: error => `表头错误：${error}`,
      importCounts: (valid, invalid, duplicate) => `${valid} 有效 · ${invalid} 无效 · ${duplicate} 重复`,
      importNotCommittable: '请先修正无效行再提交。',
      importColLine: '行',
      importColStatus: '状态',
      importColEvent: '事件',
      importColSymbol: '代码',

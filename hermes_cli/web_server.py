...
# Finance runs as a separate service. Keep its API behind the dashboard's
# existing authentication and same-origin policy via this narrow proxy.
...
_LOOPBACK_HOST_VALUES: frozenset = frozenset({"localhost", "127.0.0.1", "::1"}) | frozenset(
    # Operator-declared aliases that resolve to loopback on this machine, such
    # as the local /etc/hosts name ``my.hermes``.
    host.strip().lower()
    for host in os.environ.get("HERMES_DASHBOARD_EXTRA_HOSTS", "").split(",")
    if host.strip()
)

...
CONFIGURABLE_TOOLSETS = [
    ...
    ("finance",          "📈 Finance",                  "quotes, K-lines, analysis, research, portfolio drafts (no execution)"),
...
_DEFAULT_OFF_TOOLSETS = {
    ...
    "video_gen", "x_search", "a2a", "finance",
}
...
def _composite_toolsets(toolset_names: List[str], platform: str, explicitly_configured: bool) -> Set[str]:
    ...
    # Finance is service-gated and absent from every generic platform bundle.
    # Inject it only when the operator configured the separate Finance service;
    # its model tools remain read/draft-only and have their own runtime check_fn.
    if _toolset_allowed_for_platform("finance", platform) and os.getenv("HERMES_FINANCE_SERVICE_URL"):
        enabled.add("finance")
        default_off.discard("finance")
